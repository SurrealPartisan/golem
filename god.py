...
class God(creature.Creature):
    ...
    def smite(self, target):
        if self.sin == 'gluttony':
            ...
            wornlist = [it[0] for part in target.bodyparts for it in part.worn.values() if len(it) > 0]
            if self.power == 'powerful':
                for it in wornlist:
                    it.owner.remove(it)
                ...
                it.owner.remove(it)
                self.log().append('You stripped away the' + it.name + 'the' + target.name + ' was wearing!')
