...
STATICS_PATH = BASE_PATH.joinpath('statics')
...
win = pygcurse.PygcurseWindow(mapwidth + hpbarwidth + hpmargin,
                              mapheight + statuslines + logheight, 'Golem: A Self-Made Person!')
if file_exists('options.pickle'):
    with open('options.pickle', 'rb') as f:
        font, fontsize, showsequentially = pickle.load(f)
else:
    font, fontsize, showsequentially = (
        FONTS_PATH.joinpath('courier-prime-sans.regular.ttf'), 12, False)
...
def game():
    if SAVE_PATH.exists():
        ...
        for i in range(13):
            ct = item.CureType(
                'living flesh', medicinenames[i], i-2, np.random.choice([1, 5, 10, 25, 50]))
            curetypes.append(ct)
            drugtypes.append(ct)
        infusionnames = []
        ...
        for i in range(13):
            ct = item.CureType(
                'undead flesh', infusionnames[i], i-2, np.random.randint(1, 500))
            curetypes.append(ct)
        ...
        for i in range(numlevels):
            ...
            for j in range(10):
                ...
                while cave.walls[x, y] != 0 or cave.lavapits[x, y] != 0 or cave.campfires[x, y] != 0:
                    x = np.random.randint(mapwidth)
                    y = np.random.randint(mapheight)
                item.Cure(cave.items, x, y, drugtypes[np.random.randint(
                    13)], np.random.randint(max(1, i), i+3))
            ...
            for j in range(5):
                ...
                while cave.walls[x, y] != 0 or cave.lavapits[x, y] != 0 or cave.campfires[x, y] != 0:
                    x = np.random.randint(mapwidth)
                    y = np.random.randint(mapheight)
                item.Cure(cave.items, x, y, infusiontypes[np.random.randint(
                    13)], np.random.randint(max(1, i), i+3))
            ...
            for j in range(np.random.randint(0, 6)):
                ...
                if np.random.rand() < 0.2:
                    ...
                    spell = magic.randomspell(i+1)
                item.Spellbooklet(cave.items, x, y, spell)
            ...
            for j in range(10):
                ...
                p = p/sum(p)
                while cave.walls[x, y] != 0 or cave.lavapits[x, y] != 0 or cave.campfires[x, y] != 0 or (x, y) == cave.stairsupcoords:
                    x = np.random.randint(mapwidth)
                    y = np.random.randint(mapheight)
                cave.creatures.append(np.random.choice(
                    enemytypes, p=p)(cave, i, x, y))
    ...
    def draw():
        ...
        for i in range(mapwidth + hpbarwidth + hpmargin):
            for j in range(mapheight + statuslines + logheight):
                ...
                win.putchars(' ', x=i, y=j, bgcolor='black')
        if not player.dead:
            # This should prevent at least some of the lag when the player has no brain.
            seen = player.seen()
        else:
            seen = []
            for i in range(numlevels):
                seen.append([[(' ', (255, 255, 255), (0, 0, 0), (0, 0, 0))]
                            * mapheight for i in range(mapwidth)])
            ...
            for j in range(mapheight):
                if fovmap[i, j]:
                    ...
                    if cave.walls[i, j]:
                        ...
                        win.putchars('#', x=i, y=j, bgcolor=(
                            64, 64, 64), fgcolor='white')
                        seen[cave_i][i][j] = (
                        ...
                        win.putchars('%', x=i, y=j, bgcolor=(
                            64, 64, 64), fgcolor=(255, 204, 0))
                        seen[cave_i][i][j] = (
                            '%', (128, 102, 0), (0, 0, 0), (0, 0, 0))
                    else:
                        win.putchars(' ', x=i, y=j, bgcolor=visiblebgcolor)
                        seen[cave_i][i][j] = (
                            ' ', (255, 255, 255), nonvisiblebgcolor, (0, 0, 0))
                else:
                    win.putchars(seen[cave_i][i][j][0], x=i, y=j,
                                 fgcolor=seen[cave_i][i][j][1], bgcolor=seen[cave_i][i][j][2])
                    win.settint(seen[cave_i][i][j][3][0], seen[cave_i][i]
                                [j][3][1], seen[cave_i][i][j][3][2], (i, j, 1, 1))
        ...
        for it in cave.items:
            if fovmap[it.x, it.y]:
                if not player.dead and not it in player.itemsseen() and not it.hidden:
                    brains = [
                        part for part in player.bodyparts if 'brain' in part.categories and not part.destroyed()]
                    if len(brains) > 0:
                ...
                if player.dead or it in player.itemsseen() or not it.hidden:
                    ...
                    seen[cave_i][it.x][it.y] = (
                        '?', (128, 128, 128), nonvisiblebgcolor, (0, 0, 0))
            ...
            for j in range(mapheight):
                if fovmap[i, j]:
                    if cave.poisongas[i, j]:
                        ...
                        seen[cave_i][i][j] = (
                            seen[cave_i][i][j][0], seen[cave_i][i][j][1], seen[cave_i][i][j][2], (0, 64, 0))

        # Status
        for i in range(mapwidth):
            for j in range(statuslines):
                win.putchars(' ', x=i, y=mapheight+j,
                             bgcolor=((128, 128, 128)))
        for i in range(hpmargin):
            for j in range(mapheight + statuslines + logheight):
                win.putchars(' ', x=mapwidth+i, y=j, bgcolor=((128, 128, 128)))
        if not player.dying():
            win.putchars('HP', x=mapwidth + hpmargin +
                         hpbarwidth//2 - 1, y=0, fgcolor='white')
            hptext = repr(int(sum([part.maxhp for part in player.bodyparts])/2-sum(
                [part.damagetaken for part in player.bodyparts]))) + '/' + repr(int(sum([part.maxhp for part in player.bodyparts])/2))
            win.putchars('total:', x=mapwidth + hpmargin,
                         y=1, fgcolor=(255, 0, 0))
            win.putchars(hptext, x=mapwidth + hpmargin +
                         hpbarwidth - len(hptext), y=2, fgcolor=(255, 0, 0))
            texty = 3
            for part in player.bodyparts:
                ...
                win.putchars(shortname + ':', x=mapwidth +
                             hpmargin, y=texty, fgcolor=textcolor)
                hptext = repr(part.maxhp - part.damagetaken) + \
                    '/' + repr(part.maxhp)
                win.putchars(hptext, x=mapwidth + hpmargin + hpbarwidth -
                             len(hptext), y=texty+1, fgcolor=textcolor)
                texty += 2

        win.putchars(player.individualname + ' the golem', x=0,
                     y=mapheight, fgcolor='white', bgcolor=(128, 128, 128))
        win.putchars('Int: ' + repr(player.intelligence()), x=30,
                     y=mapheight, fgcolor='white', bgcolor=(128, 128, 128))
        win.putchars('Mana: ' + repr(int(player.mana())) + '/' + repr(int(player.manacapacity())),
                     x=45, y=mapheight, fgcolor='white', bgcolor=(128, 128, 128))
        win.putchars('Score: ' + repr(player.xp), x=65, y=mapheight,
                     fgcolor='white', bgcolor=(128, 128, 128))
        win.putchars('Dungeon level ' + repr(cave_i + 1), x=84,
                     y=mapheight, fgcolor='white', bgcolor=(128, 128, 128))
        ...
        if not player.dying():
            ...
            for effect in statuseffects:
                if textx + len(effect[0]) > mapwidth:
                    textx = 0
                    texty += 1
                win.putchars(effect[0], x=textx, y=texty, bgcolor=(
                    (128, 128, 128)), fgcolor=effect[1])
                textx += len(effect[0]) + 1
        else:
            win.putchars('Dead', x=textx, y=texty, bgcolor=(
                (128, 128, 128)), fgcolor=(255, 0, 0))
        ...
        if gamestate == 'free' or gamestate == 'dead' or gamestate == 'win':
            ...
            win.write(minemessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))

        elif gamestate == 'pick':
            pickmessage = 'Choose the item to pick:'
            win.write(pickmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if picklist[j].maxhp < np.inf:
                    itname = num + picklist[j].name + ' (wt: ' + repr(picklist[j].weight) + ' g,' + ' hp: ' + repr(
                        picklist[j].hp()) + '/' + repr(picklist[j].maxhp) + ')'
                else:
                    itname = num + picklist[j].name + \
                        ' (wt: ' + repr(picklist[j].weight) + ' g)'
                if j != chosen:
                    win.write(itname, x=0, y=mapheight+statuslines +
                              i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(itname, x=0, y=mapheight+statuslines +
            ...
            win.write(dropmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if player.inventory[j].maxhp < np.inf:
                    itname = num + player.inventory[j].name + ' (wt: ' + repr(player.inventory[j].weight) + ' g,' + ' hp: ' + repr(
                        player.inventory[j].hp()) + '/' + repr(player.inventory[j].maxhp) + ')'
                else:
                    itname = num + \
                        player.inventory[j].name + \
                        ' (wt: ' + repr(player.inventory[j].weight) + ' g)'
                if j != chosen:
                    win.write(itname, x=0, y=mapheight+statuslines +
                              i+1, fgcolor=(255, 255, 255))
                if j == chosen:
            ...
            win.write(infomessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if j < 9:
                    ...
                    num = ''
                if isinstance(itemlist[j], item.Item):
                    if itemlist[j].maxhp < np.inf:
                        itname = num + itemlist[j].name + ' (wt: ' + repr(itemlist[j].weight) + ' g,' + ' hp: ' + repr(
                            itemlist[j].hp()) + '/' + repr(itemlist[j].maxhp) + ')'
                    else:
                        itname = num + \
                            itemlist[j].name + \
                            ' (wt: ' + repr(itemlist[j].weight) + ' g)'
                else:  # Spells
                    itname = num + itemlist[j].name
                if j != chosen:
                    win.write(itname, x=0, y=mapheight+statuslines +
                              i+1, fgcolor=(255, 255, 255))
                if j == chosen:
            ...
            win.write(consumemessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(
                logheight-1, len([item for item in player.inventory if item.consumable]))
            for i in range(logrows):
                if len([item for item in player.inventory if item.consumable]) <= logheight-1:
                    j = i
                else:
                    j = len(
                        [item for item in player.inventory if item.consumable])+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                ...
                if it.cure and it.curetype in player.curesknown():
                    ...
                    if it.hpgiven() >= 0:
                        itemdescription += '+'
                    itemdescription += repr(it.hpgiven()) + \
                        ' hp to ' + it.curedmaterial + ')'
                if j != chosen:
                    win.write(itemdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
            ...
            win.write(cookmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(
                logheight-1, len([item for item in player.inventory if item.material == 'living flesh']))
            for i in range(logrows):
                if len([item for item in player.inventory if item.material == 'living flesh']) <= logheight-1:
                    j = i
                else:
                    j = len([item for item in player.inventory if item.material ==
                            'living flesh'])+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                it = [item for item in player.inventory if item.material ==
                      'living flesh'][j]
                itemdescription = num + it.name
                if j != chosen:
                    win.write(itemdescription, x=0, y=mapheight +
            ...
            win.write(wieldmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(
                logheight-1, len([item for item in player.inventory if item.wieldable]))
            for i in range(logrows):
                if len([item for item in player.inventory if item.wieldable]) <= logheight-1:
                    j = i
                else:
                    j = len(
                        [item for item in player.inventory if item.wieldable])+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + [item for item in player.inventory if item.wieldable]
                              [j].name, x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + [item for item in player.inventory if item.wieldable][j].name,
                              x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'wieldchoosebodypart':
            wieldmessage = 'Choose where to wield the ' + selecteditem.name + ':'
            win.write(wieldmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(logheight-1, len([part for part in player.bodyparts if part.capableofwielding and len(
                part.wielded) == 0 and not part.destroyed()]))
            for i in range(logrows):
                if len([part for part in player.bodyparts if part.capableofwielding and len(part.wielded) == 0 and not part.destroyed()]) <= logheight-1:
                    ...
                    j = len([part for part in player.bodyparts if part.capableofwielding and len(
                        part.wielded) == 0 and not part.destroyed()])+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + [part.wearwieldname() for part in player.bodyparts if part.capableofwielding and len(
                        part.wielded) == 0 and not part.destroyed()][j], x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + [part.wearwieldname() for part in player.bodyparts if part.capableofwielding and len(part.wielded)
                              == 0 and not part.destroyed()][j], x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            win.write(wieldmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(
                logheight-1, len([part for part in player.bodyparts if part.capableofwielding and len(part.wielded) > 0]))
            for i in range(logrows):
                if len([part for part in player.bodyparts if part.capableofwielding and len(part.wielded) > 0]) <= logheight-1:
                    j = i
                else:
                    j = len([part for part in player.bodyparts if part.capableofwielding and len(
                        part.wielded) > 0])+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + [part.wielded[0].name + ' in the ' + part.wearwieldname() for part in player.bodyparts if part.capableofwielding and len(
                        part.wielded) > 0][j], x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + [part.wielded[0].name + ' in the ' + part.wearwieldname() for part in player.bodyparts if part.capableofwielding and len(
                        part.wielded) > 0][j], x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'wearchooseitem':
            wearmessage = 'Choose the item to wear:'
            win.write(wearmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(
                logheight-1, len([item for item in player.inventory if item.wearable]))
            for i in range(logrows):
                if len([item for item in player.inventory if item.wearable]) <= logheight-1:
                    j = i
                else:
                    j = len(
                        [item for item in player.inventory if item.wearable])+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + [item for item in player.inventory if item.wearable]
                              [j].name, x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + [item for item in player.inventory if item.wearable][j].name,
                              x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            for i in range(logrows):
                ...
                if j < 9:
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + partlist[j].wearwieldname() + ' (' + selecteditem.wearcategory +
                              ' slot)', x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + partlist[j].wearwieldname() + ' (' + selecteditem.wearcategory + ' slot)',
                              x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'undress':
            choosemessage = 'Choose the item to take off:'
            win.write(choosemessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            wornlist = [
                it[0] for part in player.bodyparts for it in part.worn.values() if len(it) > 0]
            ...
            for i in range(logrows):
                ...
                if j < 9:
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + wornlist[j].name + ' on the ' + wornlist[j].owner.owner.wearwieldname(
                    ), x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + wornlist[j].name + ' on the ' + wornlist[j].owner.owner.wearwieldname(
                    ), x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            win.write(attackmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if j < 9:
                    ...
                    num = ''
                attacktime = player.attackslist(
                )[j].time * (1 + player.slowed())
                ...
                attackdescription = num + player.attackslist()[j].name + ' (' + repr(int(attackerstancecoefficient*highgroundcoefficient*player.attackslist()[j].hitprobability * 100)) + '%, ' + repr(
                    player.attackslist()[j].mindamage) + '-' + repr(player.attackslist()[j].maxdamage) + ' ' + player.attackslist()[j].damagetype + ', ' + repr(attacktime) + ' s'
                for special in player.attackslist()[j].special:
                    if special[0] == 'bleed':
                        attackdescription += ', bleed ' + \
                            repr(int(special[1] * 100)) + '%'
                    if special[0] == 'knockback':
                        attackdescription += ', knockback ' + \
                            repr(int(special[1] * 100)) + '%'
                    if special[0] == 'charge':
                ...
                if j != chosen:
                    win.write(attackdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(attackdescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            for i in range(logrows):
                ...
                if player.previousaction[0] == 'fight' and player.previousaction[1] == selectedattack.weapon and player.previousaction[2] == targetbodypart:
                    targetdescription = num + partname + ' (' + repr(int(defenderstancecoefficient*heightcoefficient*protectioncoefficient *
                                                                         speedcoefficient*imbalancedcoefficient*targetbodypart.defensecoefficient() * 100)) + '%, time x1.5)'
                else:
                    targetdescription = num + partname + ' (' + repr(int(defenderstancecoefficient*heightcoefficient*protectioncoefficient *
                                                                         speedcoefficient*imbalancedcoefficient*targetbodypart.defensecoefficient() * 100)) + '%)'
                if targetbodypart.incapacitated():
                    targetdescription += ' [INCAPACITATED]'
                if j != chosen:
                    win.write(targetdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(targetdescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            for i in range(logrows):
                ...
                if player.stance == 'aggressive':
                    ...
                    attackerstancecoefficient = 1
                attackdescription = num + player.thrownattackslist()[j].name + ' (' + repr(int(attackerstancecoefficient*player.thrownattackslist()[j].hitprobability * 100)) + '%, ' + repr(player.thrownattackslist(
                )[j].mindamage) + '-' + repr(player.thrownattackslist()[j].maxdamage) + ' ' + player.thrownattackslist()[j].damagetype + ', ' + repr(int(player.thrownattackslist()[j].weapon.throwrange)) + ' paces'
            ...
            fovmap = player.fov()
            fovmap2 = fov(cave.walls, player.x, player.y,
                          selectedattack.weapon.throwrange, nowalls=True)
            for i in range(mapwidth):
                for j in range(mapheight):
            ...
            instructions = 'Move the cursor with the movement keys to choose the target, then press Return. ESC to cancel.'
            win.write(instructions, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if target.imbalanced():
                    ...
                    imbalancedcoefficient = 1
                targetdescription = num + partname + ' (' + repr(int(defenderstancecoefficient*protectioncoefficient *
                                                                     speedcoefficient*imbalancedcoefficient*targetbodypart.defensecoefficient() * 100)) + '%)'
                if targetbodypart.incapacitated():
                    targetdescription += ' [INCAPACITATED]'
                if j != chosen:
            ...
            for i in range(logrows):
                ...
                if limb.parentalconnection != None:
                    partname = list(limb.parentalconnection.parent.childconnections.keys())[list(
                        limb.parentalconnection.parent.childconnections.values()).index(limb.parentalconnection)]
                ...
                if (player.stance == 'flying' or cave.largerocks[player.x, player.y]) and target.stance != 'flying' and not cave.largerocks[target.x, target.y]:
                    ...
                    highgroundcoefficient = 1
                distance = np.sqrt(
                    (targetcoords[0] - player.x)**2 + (targetcoords[1] - player.y)**2)
                wornlist = [
                    it[0] for part in player.bodyparts for it in part.worn.values() if len(it) > 0]
                notwielded = selectedattack.weapon.owner == player.inventory and not np.any(
                    [hasattr(it, 'quickdraw') and it.quickdraw for it in wornlist])
                throwtime = (1/limb.throwspeed + notwielded) * \
                    (1 + player.slowed())
                if int(throwtime) == throwtime:
                    throwtime = int(throwtime)
                else:
                    throwtime = round(throwtime, 2)
                limbdescription = num + partname + \
                    ' (' + repr(int(highgroundcoefficient*limb.throwaccuracy **
                                    distance * 100)) + '%, ' + repr(throwtime) + ' s)'
                if j != chosen:
                    win.write(limbdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(limbdescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            win.write(choosemessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            torsolist = [part for part in player.bodyparts if 'torso' in part.categories and part.usable and not part.destroyed(
            )] + [it for it in player.inventory if it.bodypart and 'torso' in it.categories and it.usable and not it.destroyed()]
            ...
            for i in range(logrows):
                ...
                if j < 9:
                    ...
                    num = ''
                if j != chosen:
                    win.write(num + torsolist[j].name + ' (hp: ' + repr(torsolist[j].hp()) + '/' + repr(
                        torsolist[j].maxhp) + ')', x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + torsolist[j].name + ' (hp: ' + repr(torsolist[j].hp()) + '/' + repr(
                        torsolist[j].maxhp) + ')', x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
            ...
            for i in range(logrows):
                ...
                if j == 0:
                    part = [
                        part for part in bodypartcandidates if 'torso' in part.categories][0]
                    connectiondescription = 'torso: ' + part.name + \
                        ' (hp: ' + repr(part.hp()) + '/' + repr(part.maxhp) + ')'
                    fgcolorchosen = (0, 0, 0)
                    ...
                    connectionname2 = list(connectioncandidates[j][0].parent.childconnections.keys())[list(
                        connectioncandidates[j][0].parent.childconnections.values()).index(connectioncandidates[j][0])]
                    part = connectioncandidates[j][1]
                    if part != None:
                        connectiondescription = connectionname2 + ': ' + part.name + \
                            ' (hp: ' + repr(part.hp()) + \
                            '/' + repr(part.maxhp) + ')'
                    else:
                    ...
                    if connectioncandidates[j][0].vital and connectioncandidates[j][1] == None:
                        ...
                        fgcolorunchosen = (255, 255, 255)
                if j != chosen:
                    win.write(num + connectiondescription, x=0,
                              y=mapheight+statuslines+i+1, fgcolor=fgcolorunchosen)
                if j == chosen:
                    win.write(num + connectiondescription, x=0, y=mapheight +
                              statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=fgcolorchosen)
            ...
            if logrows == 0:
                win.write('No suitable options for a vital bodypart. Press ESC to cancel.',
            ...
            for i in range(logrows):
                ...
                if partslist[j] == None:
                    ...
                    partdescription = partslist[j].name + ' (hp: ' + repr(
                        partslist[j].hp()) + '/' + repr(partslist[j].maxhp) + ')'
                if j != chosen:
                    win.write(num + partdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + partdescription, x=0, y=mapheight +
                              statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'choosestance':
            stancemessage = 'Which stance do you want to be in'
            win.write(stancemessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if j != chosen:
                    win.write(num + stancetext, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(num + stancetext, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'pray':
            praymessage = 'To which sinful god of the underground do you wish to pray?'
            win.write(praymessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                if j != chosen:
                    if j < len(player.godsknown()):
                        win.write(num + player.godsknown()[j].name + ', the ' + player.godsknown()[j].factions[0] + '-god of ' + player.godsknown()[
                                  j].sin + ' (' + player.godsknown()[j].power + ', ' + player.godsknown()[j].attitude + ')', x=0, y=mapheight+statuslines+i+1, fgcolor=(255, 255, 255))
                    else:
                        win.write(num + 'whomever listens', x=0, y=mapheight +
                                  statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    if j < len(player.godsknown()):
                        win.write(num + player.godsknown()[j].name + ', the ' + player.godsknown()[j].factions[0] + '-god of ' + player.godsknown()[j].sin + ' (' + player.godsknown()[
                                  j].power + ', ' + player.godsknown()[j].attitude + ')', x=0, y=mapheight+statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))
                    else:
                        win.write(num + 'whomever listens', x=0, y=mapheight +
                                  statuslines+i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'choosespell':
            spellmessage = 'Choose the spell to cast:'
            win.write(spellmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(logheight-1, len(spelllist))
            for i in range(logrows):
                if len(spelllist) <= logheight-1:
                    j = i
                else:
                    j = len(spelllist)+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                spell = spelllist[j]
                spelldescription = num + spell.name + \
                    ' (' + repr(spell.manarequirement) + \
                    ' mana, ' + repr(spell.castingtime) + ' s)'
                if j != chosen:
                    win.write(spelldescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(spelldescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'spellmakechoice':
            win.write(selectedspell.choiceprompt, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            ...
            for i in range(logrows):
                ...
                choicedescription = num + \
                    selectedspell.choicedescription(player, choice)
                if j != chosen:
                    win.write(choicedescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(choicedescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'spellchooselocation':
            for i in range(mapwidth):
                for j in range(mapheight):
                    if np.sqrt((i - lookx)**2 + (j - looky)**2) < selectedspell.radius:
                        win.settint(64, 64, 0, (i, j, 1, 1))
            win.settint(128, 128, 0, (lookx, looky, 1, 1))

            win.write(selectedspell.locationchoiceprompt, x=0,
                      y=mapheight+statuslines, fgcolor=(0, 255, 255))
            if selectedspell.suitablelocation(cave, lookx, looky):
                win.write(selectedspell.suitablelocationmessage, x=0,
                          y=mapheight+statuslines+1, fgcolor=(255, 255, 255))
            else:
                win.write(selectedspell.unsuitablelocationmessage, x=0,
                          y=mapheight+statuslines+1, fgcolor=(255, 255, 255))

        elif gamestate == 'spellchoosetarget':
            win.settint(128, 128, 0, (lookx, looky, 1, 1))

            win.write(selectedspell.targetchoiceprompt, x=0,
                      y=mapheight+statuslines, fgcolor=(0, 255, 255))
            if len([creat for creat in cave.creatures if creat.x == lookx and creat.y == looky and creat != player]) > 0:
                win.write(selectedspell.suitabletargetmessage(
                    [creat for creat in cave.creatures if creat.x == lookx and creat.y == looky and creat != player][0]), x=0, y=mapheight+statuslines+1, fgcolor=(255, 255, 255))
            else:
                win.write(selectedspell.notargetmessage, x=0,
                          y=mapheight+statuslines+1, fgcolor=(255, 255, 255))

        elif gamestate == 'spellchoosetargetbodypart':
            win.write(selectedspell.partchoiceprompt(target), x=0,
                      y=mapheight+statuslines, fgcolor=(0, 255, 255))
            logrows = min(logheight-1, len(selectedspell.partchoices(target)))
            for i in range(logrows):
                if len(selectedspell.partchoices(target)) <= logheight-1:
                    j = i
                else:
                    j = len(selectedspell.partchoices(target)) + \
                        i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    ...
                    num = ''
                part = selectedspell.partchoices(target)[j]
                partdescription = num + \
                    selectedspell.partdescription(target, part, player)
                if j != chosen:
                    win.write(partdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(partdescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'read':
            readmessage = 'Choose the booklet to read:'
            win.write(readmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(logheight-1, len(readlist))
            for i in range(logrows):
                if len(readlist) <= logheight-1:
                    j = i
                else:
                    j = len(readlist)+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
                    ...
                    num = ''
                booklet = readlist[j]
                bookletdescription = num + booklet.name + \
                    ' (intelligence ' + \
                    repr(booklet.spell.intelligencerequirement) + ' required)'
                if j != chosen:
                    win.write(bookletdescription, x=0, y=mapheight +
                              statuslines+i+1, fgcolor=(255, 255, 255))
                if j == chosen:
                    win.write(bookletdescription, x=0, y=mapheight+statuslines +
                              i+1, bgcolor=(255, 255, 255), fgcolor=(0, 0, 0))

        elif gamestate == 'write':
            spellmessage = 'Choose the spell to write:'
            win.write(spellmessage, x=0, y=mapheight +
                      statuslines, fgcolor=(0, 255, 255))
            logrows = min(logheight-1, len(player.spellsknown()))
            for i in range(logrows):
                if len(player.spellsknown()) <= logheight-1:
                    j = i
                else:
                    j = len(player.spellsknown())+i-logrows-logback
                if j < 9:
                    num = repr(j + 1) + ': '
                elif j == 9:
                    num = '0: '
    ...
    def _updatetime(time):
        ...
        if player.stance != 'running' and not player.hungry():
            player.runstaminaused = max(
                0, player.runstaminaused - time*player.runstaminarecoveryspeed())
        elif player.stance != 'running' and not player.starving():
            player.runstaminaused = max(
                0, player.runstaminaused - time*0.5*player.runstaminarecoveryspeed())
        if player.panicked():
        ...
        if cave.poisongas[player.x, player.y]:
            livingparts = [part for part in player.bodyparts if part.material ==
                           'living flesh' and not part.destroyed()]
            lungs = [
                part for part in player.bodyparts if 'lung' in part.categories and not part.destroyed()]
            if np.random.rand() > player.breathepoisonresistance() and len(livingparts) > 0 and len(lungs) > 0:
                ...
                player.accumulatedpoison = min(
                    50, player.accumulatedpoison + np.random.rand()*40)
    ...
    def moveorattack(dx, dy):
        ...
        targets = [creature for creature in cave.creatures if creature.x ==
                   player.x+dx and creature.y == player.y+dy]
        if len(targets) > 0 and not stumbling:
            ...
            updatetime((np.sqrt(dx**2 + dy**2) * player.steptime() * (1 + player.slowed()) * (
                1 + (cave.largerocks[player.x+dx, player.y+dy] and player.stance != 'flying')))/2)
            if not player.dying():
                if player.stance == 'running':
                    player.runstaminaused += np.sqrt(dx**2 + dy**2) * player.steptime() * (1 + player.slowed()) * (1 + (
                        cave.largerocks[player.x+dx, player.y+dy] and player.stance != 'flying')) * (1 + player.burdened()) / 2
                if player.stance != 'running':
                    ...
                    if 'leg' in part.categories:
                        numlegs = len([p for p in player.bodyparts if 'leg' in p.categories and not p.destroyed(
                    ...
                    if not player.dying():
                        if not part.destroyed():
                            player.log().append('You ran into wall. It dealt ' + repr(damage) +
                            ...
                            player.log().append('You ran into wall. It destroyed your ' +
            ...
            gamestate = 'free'
            updatetime(np.sqrt(dx**2 + dy**2) * player.steptime() * (1 + player.slowed()) * (
                1 + (cave.largerocks[player.x+dx, player.y+dy] and player.stance != 'flying')))
            if not player.dying():
                if player.stance == 'running':
                    player.runstaminaused += np.sqrt(dx**2 + dy**2) * player.steptime() * (1 + player.slowed()) * (1 + (
                        cave.largerocks[player.x+dx, player.y+dy] and player.stance != 'flying')) * (1 + player.burdened())
                creaturesintheway = [
                    creature for creature in cave.creatures if creature.x == player.x+dx and creature.y == player.y+dy]
                if len(creaturesintheway) == 0:
                    player.move(dx, dy)
                    player.previousaction = ('move',)
                else:
                    player.log().append("There's a " +
    ...
    def mine(dx, dy):
        if player.x+dx == 0 or player.x+dx == mapwidth-1 or player.y+dy == 0 or player.y+dy == mapheight-1:
            ...
            if not player.dying():
                ...
                found = np.random.choice(
                    [0, 1, 2, 3], p=[0.79-0.01*(cave_i % 5), 0.1, 0.1, 0.01+0.01*(cave_i % 5)])
                if found == 1:
                    cave.largerocks[player.x+dx, player.y+dy] = 1
                if found == 2:
                    pebbles = item.LooseRoundPebbles(
                        cave.items, player.x+dx, player.y+dy)
                    player.itemsseen().append(pebbles)
                if found == 3:
                    if cave_i < 5:
                        bodypart.VelociraptorSkull(
                            cave.items, player.x+dx, player.y+dy)
                    elif cave_i < 10:
                        bodypart.DeinonychusSkull(
                            cave.items, player.x+dx, player.y+dy)
                    elif cave_i < 15:
                        bodypart.CeratosaurusSkull(
                            cave.items, player.x+dx, player.y+dy)
                    elif cave_i < 20:
                        bodypart.AllosaurusSkull(
                            cave.items, player.x+dx, player.y+dy)
                    else:
            ...
            if event.type == pygame.locals.KEYDOWN:
                ...
                if player.dying() and gamestate != 'dead':
                    ...
                    if player.causeofdeath[0] == 'attack':
                        deathmessage = 'killed by a ' + \
                            player.causeofdeath[1].name
                    elif player.causeofdeath[0] == 'smite':
                        deathmessage = 'smitten to death by ' + \
                            player.causeofdeath[1].name + ', the ' + \
                            player.causeofdeath[1].factions[0] + \
                            '-god of ' + player.causeofdeath[1].sin
                    elif player.causeofdeath[0] == 'bloodloss':
                        deathmessage = 'bled to death after being attacked by a ' + \
                            ' and a '.join(
                                [creat.name for creat in player.causeofdeath[1]])
                        ...
                        deathmessage = 'died from adverse effects of ' + \
                            player.causeofdeath[1].curetype.name
                    elif player.causeofdeath[0] == 'burning':
                        deathmessage = 'burned to death ' + \
                            player.causeofdeath[1]
                    elif player.causeofdeath[0] == 'step':
                        deathmessage = 'died from stepping on ' + \
                            player.causeofdeath[1].name
                    elif player.causeofdeath[0] == 'trip':
                        deathmessage = 'died from tripping on ' + \
                            player.causeofdeath[1].name
                    elif player.causeofdeath[0] == 'ranintowall':
                        deathmessage = 'died from running into wall ' + \
                            player.causeofdeath[1] + ' first'
                    else:
                        deathmessage = 'died from unknown causes'
                    deathmessage += (' on dungeon level ' +
                                     repr(player.world_i+1) + '.')
                    highscores.append(
                        (player.xp, player.individualname, deathmessage))
                ...
                if gamestate == 'free':
                    ...
                    if (event.key == keybindings['repeat last attack'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['repeat last attack'][0][1])) or (event.key == keybindings['repeat last attack'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['repeat last attack'][1][1])):
                        if lastattack != None:
                            ...
                            if not target.dead and abs(target.x - player.x) <= 1 and abs(target.y - player.y) <= 1 and selected in target.bodyparts and not selected.destroyed() and selectedattack in player.attackslist():
                                ...
                                numchosen = False
                                updatetime(
                                    selectedattack[6] * repetitionmultiplier * (1 + player.slowed()))
                                if not player.dying():
                                    if not target.dead:
                                        player.fight(
                                            target, selected, selectedattack)
                                        player.previousaction = (
                                            'fight', selectedattack.weapon, selected)
                                    else:
                        ...
                        if (player.x, player.y) != cave.stairsdowncoords:
                            ...
                            if cave_i == numlevels - 1:
                                ...
                                if file_exists('highscores.pickle'):
                                    ...
                                    highscores = []
                                highscores.append(
                                    (player.xp, player.individualname, 'won the game!'))
                                with open('highscores.pickle', 'wb') as f:
                                    pickle.dump(highscores, f)
                                logback = 0
                            else:
                    ...
                    if (event.key == keybindings['pick up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['pick up'][0][1])) or (event.key == keybindings['pick up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['pick up'][1][1])):
                        picklist = [it.pickableinstance() for it in cave.items if abs(it.x - player.x) <= 1 and abs(
                            it.y - player.y) <= 1 and (it in player.itemsseen() or not it.hidden) and it.pickableinstance() != None]
                        ...
                        if len(picklist) == 0:
                            ...
                            distance = np.sqrt(
                                (player.x - picklist[0].x)**2 + (player.y - picklist[0].y)**2)
                            updatetime(0.5 * (1 + distance) *
                                       (1 + player.slowed()))
                            if not player.dying():
                                it = picklist[0]
                                if it.owner != None:
                    ...
                    if (event.key == keybindings['inventory'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['inventory'][0][1])) or (event.key == keybindings['inventory'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['inventory'][1][1])):
                        ...
                        if max(1, len(player.inventory)) + max(1, len(wieldlist)) + max(1, len(wornlist)) + 3 > logheight and len(player.log()) > 1:
                            logback = max(1, len(
                                player.inventory)) + max(1, len(wieldlist)) + max(1, len(wornlist)) + 3 - logheight
                        else:
                            logback = 0
                    ...
                    if (event.key == keybindings['information'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['information'][0][1])) or (event.key == keybindings['information'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['information'][1][1])):
                        itemlist = player.inventory + [part.wielded[0] for part in player.bodyparts if part.capableofwielding and len(part.wielded) > 0] + [
                            it[0] for part in player.bodyparts for it in part.worn.values() if len(it) > 0] + player.bodyparts + player.spellsknown()
                    ...
                    if (event.key == keybindings['consume'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['consume'][0][1])) or (event.key == keybindings['consume'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['consume'][1][1])):
                        if len([item for item in player.inventory if item.consumable]) > 0:
                            gamestate = 'consume'
                            logback = len(
                                [item for item in player.inventory if item.consumable]) - logheight + 1
                            chosen = 0
                            numchosen = False
                        else:
                    ...
                    if (event.key == keybindings['cook'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['cook'][0][1])) or (event.key == keybindings['cook'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['cook'][1][1])):
                        ...
                        if not campfiresnear:
                            ...
                            gamestate = 'cook'
                            logback = len(
                                [item for item in player.inventory if item.material == 'living flesh']) - logheight + 1
                            chosen = 0
                            numchosen = False
                    ...
                    if (event.key == keybindings['wield'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['wield'][0][1])) or (event.key == keybindings['wield'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['wield'][1][1])):
                        if len([item for item in player.inventory if item.wieldable]) > 0 and len([part for part in player.bodyparts if part.capableofwielding and len(part.wielded) == 0 and not part.destroyed()]) > 0:
                            ...
                            logback = len(
                                [item for item in player.inventory if item.wieldable]) - logheight + 1
                            chosen = 0
                            numchosen = False
                        else:
                    ...
                    if (event.key == keybindings['wear'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['wear'][0][1])) or (event.key == keybindings['wear'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['wear'][1][1])):
                        if len([item for item in player.inventory if item.wearable]) > 0:
                            ...
                            logback = len(
                                [item for item in player.inventory if item.wearable]) - logheight + 1
                            chosen = 0
                            numchosen = False
                        else:
                    ...
                    if (event.key == keybindings['unwield'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['unwield'][0][1])) or (event.key == keybindings['unwield'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['unwield'][1][1])):
                        if len([part for part in player.bodyparts if part.capableofwielding and len(part.wielded) > 0]) > 0:
                            gamestate = 'unwield'
                            logback = len([part for part in player.bodyparts if part.capableofwielding and len(
                                part.wielded) > 0]) - logheight + 1
                            chosen = 0
                            numchosen = False
                        else:
                    ...
                    if (event.key == keybindings['throw'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['throw'][0][1])) or (event.key == keybindings['throw'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['throw'][1][1])):
                        if len(player.thrownattackslist()) > 0:
                            ...
                            logback = len(
                                player.thrownattackslist()) - logheight + 1
                            chosen = 0
                            numchosen = False
                        else:
                    ...
                    if (event.key == keybindings['cast a spell'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['cast a spell'][0][1])) or (event.key == keybindings['cast a spell'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['cast a spell'][1][1])):
                        if len([spell for spell in player.spellsknown() if player.mana() >= spell.manarequirement]) > 0 and not player.panicked():
                            ...
                            spelllist = [spell for spell in player.spellsknown(
                            ) if player.mana() >= spell.manarequirement]
                    ...
                    if (event.key == keybindings['read'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['read'][0][1])) or (event.key == keybindings['read'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['read'][1][1])):
                        if len([it for it in player.inventory if it.readable]) > 0 and player.sight() > 0 and not player.panicked():
                            gamestate = 'read'
                            readlist = [
                                it for it in player.inventory if it.readable]
                    ...
                    if (event.key == keybindings['pray'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['pray'][0][1])) or (event.key == keybindings['pray'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['pray'][1][1])):
                        gamestate = 'pray'
                        logback = min(
                            7, len(player.godsknown()) + 1) - logheight + 1
                        chosen = 0
                        numchosen = False
                    ...
                    if (event.key == keybindings['list bodyparts'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list bodyparts'][0][1])) or (event.key == keybindings['list bodyparts'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list bodyparts'][1][1])):
                        ...
                        for part in player.bodyparts:
                            player.log().append('  - a ' + part.name + ' (hp: ' + repr(part.hp()) + '/' + repr(part.maxhp) +
                                                ', ' + repr(part.bottomheight()) + ' to ' + repr(part.topheight()) + ' cm from ground)')
                    ...
                    if (event.key == keybindings['choose bodyparts'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['choose bodyparts'][0][1])) or (event.key == keybindings['choose bodyparts'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['choose bodyparts'][1][1])):
                        ...
                        bodypartcandidates = listwithowner(
                            [part for part in player.bodyparts], player)
                        connectioncandidates = [None] + [(connection, connection.child)
                                                         for part in bodypartcandidates for connection in part.childconnections.values()] + [None]
                    ...
                    if (event.key == keybindings['help'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['help'][0][1])) or (event.key == keybindings['help'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['help'][1][1])):
                        ...
                        player.log().append('  - M: cast a spell')
                        player.log().append('  - L: read')
                        player.log().append('  - S: write')
                        player.log().append('  - h: this list of commands')
                        if len(player.log()) > 1:  # Prevent crash if the player is brainless
                            logback = 21  # Increase when adding commands
                    ...
                    if (event.key == keybindings['log up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['log up'][0][1])) or (event.key == keybindings['log up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['log up'][1][1])):
                        if len(player.log()) >= logheight:
                            logback = min(
                                logback+1, len(player.log())-logheight)
                    ...
                    if (event.key == keybindings['save and exit'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['save and exit'][0][1])) or (event.key == keybindings['save and exit'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['save and exit'][1][1])):
                        ...
                        win.putchars(text, x=(mapwidth + hpmargin + hpbarwidth)//2 - len(
                            text)//2, y=(mapheight + statuslines + logheight)//2, fgcolor='white')
                        win.update()
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        gamestate = 'free'
                        distance = np.sqrt(
                            (player.x - picklist[chosen].x)**2 + (player.y - picklist[chosen].y)**2)
                        updatetime(0.5 * (1 + distance) *
                                   (1 + player.slowed()))
                        if not player.dying():
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        player.log().append('Information about the ' +
                                            selected.name + ': ' + selected.info())
                        logback = 0
                        gamestate = 'free'
                        numchosen = False
                    if (event.key == keybindings['escape list'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['escape list'][0][1])) or (event.key == keybindings['escape list'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['escape list'][1][1])):
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len([item for item in player.inventory if item.consumable])
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if not player.dying():
                            selected = [
                                item for item in player.inventory if item.consumable][chosen]
                            if selected.cure:
                                ...
                                stomachs = [
                                    part for part in player.bodyparts if 'stomach' in part.categories and not part.destroyed()]
                                if len(stomachs) > 0:
                                    stomach = stomachs[0]
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len([item for item in player.inventory if item.material == 'living flesh'])
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if not player.dying():
                            ...
                            if newname[-11:] == ' [UNUSABLE]':
                                newname = newname[:-11]
                            item.Food(player.inventory, 0, 0, newname, selected.char, ((
                                97+selected.color[0])//2, (23+selected.color[1])//2, (23+selected.color[2])//2), selected.maxhp, 'cooked meat', selected.weight)
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len([item for item in player.inventory if item.wieldable])
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        selecteditem = [
                            item for item in player.inventory if item.wieldable][chosen]
                        logback = len(
                            [part for part in player.bodyparts if part.capableofwielding]) - logheight + 1
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (chosen-1) % len([part for part in player.bodyparts if part.capableofwielding and len(
                            part.wielded) == 0 and not part.destroyed()])
                    ...
                    if (event.key == keybindings['list down'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list down'][0][1])) or (event.key == keybindings['list down'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list down'][1][1])):
                        ...
                        if chosen == 0:
                            logback = len([part for part in player.bodyparts if part.capableofwielding and len(
                                part.wielded) == 0 and not part.destroyed()]) - logheight + 1
                    for i in range(10):
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        gamestate = 'free'
                        numchosen = False
                        selected = [part for part in player.bodyparts if part.capableofwielding and len(
                            part.wielded) == 0 and not part.destroyed()][chosen]
                        updatetime(1 * (1 + player.slowed()))
                        if not player.dying():
                            if not selected.destroyed():
                                ...
                                player.log().append('You are now wielding the ' + selecteditem.name +
                                                    ' in your ' + selected.wearwieldname() + '.')
                                selecteditem.on_wearwield(player)
                            else:
                                player.log().append('Your ' + selected.wearwieldname() +
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len([part for part in player.bodyparts if part.capableofwielding and len(part.wielded) > 0])
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        gamestate = 'free'
                        numchosen = False
                        part = [part for part in player.bodyparts if part.capableofwielding and len(
                            part.wielded) > 0][chosen]
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len([item for item in player.inventory if item.wearable])
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        selecteditem = [
                            item for item in player.inventory if item.wearable][chosen]
                        partlist = [part for part in player.bodyparts if selecteditem.wearcategory in part.worn.keys(
                        ) and len(part.worn[selecteditem.wearcategory]) == 0 and not part.destroyed()]
                        if len(partlist) > 0:
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if not player.dying():
                            if not selected.destroyed():
                                player.inventory.remove(selecteditem)
                                selected.worn[selecteditem.wearcategory].append(
                                    selecteditem)
                                selecteditem.owner = selected.worn[selecteditem.wearcategory]
                                player.log().append('You are now wearing the ' + selecteditem.name +
                                                    ' on your ' + selected.wearwieldname() + '.')
                                selecteditem.on_wearwield(player)
                            else:
                                player.log().append('Your ' + selected.wearwieldname() +
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if np.any([special[0] == 'internals-seeking' for special in selectedattack.special]):
                            targetbodypartlist = [
                                part for part in target.bodyparts if not part.destroyed()]
                        else:
                            targetbodypartlist = [
                    ...
                    if (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if targetingcreature:
                            ...
                            limblist = [part for part in player.bodyparts if part.capableofwielding and part.capableofthrowing and len(
                                part.wielded) == 0]
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        notwielded = selectedattack.weapon.owner == player.inventory and not np.any(
                            [hasattr(it, 'quickdraw') and it.quickdraw for it in wornlist])
                        updatetime((1/selected.throwspeed + notwielded)
                                   * (1 + player.slowed()))
                        if not player.dying():
                            if target != None and not target.dead:
                                player.throwatenemy(
                                    target, selectedbodypart, selectedattack, selected)
                                player.previousaction = ('throw',)
                            else:
                                player.throwatsquare(
                                    targetcoords[0], targetcoords[1], selectedattack.weapon, selected)
                                player.previousaction = ('throw',)
                            detecthiddenitems()
                        logback = 0
                    if (event.key == keybindings['escape list'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['escape list'][0][1])) or (event.key == keybindings['escape list'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['escape list'][1][1])):
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        if connectioncandidates[chosen] != None:
                            ...
                            connection = None
                            # I think this could be just bodypartcandidates[0], but just in case...
                            connectedpart = [
                                part for part in bodypartcandidates if 'torso' in part.categories][0]
                        if chosen < len(connectioncandidates)-1:
                            if connection != None:
                                connectionname = list(connection.parent.childconnections.keys())[list(
                                    connection.parent.childconnections.values()).index(connection)]
                                partslist = [part for part in player.bodyparts if np.any([category in part.categories for category in connection.categories]) and part.usable and not part.destroyed() and (part == connectedpart or not part in bodypartcandidates)] + [
                                    it for it in player.inventory if it.bodypart and np.any([category in it.categories for category in connection.categories]) and it.usable and not it.destroyed() and (it == connectedpart or not it in bodypartcandidates)]
                                if not connection.vital:
                                    partslist.append(None)
                            else:
                                connectionname = 'torso'
                                partslist = [part for part in player.bodyparts if 'torso' in part.categories and part.usable and not part.destroyed(
                                )] + [it for it in player.inventory if it.bodypart and 'torso' in it.categories and it.usable and not it.destroyed()]
                            ...
                            gamestate = 'free'
                            numchosen = False
                            updatetime(bodypartchoosingtime *
                                       (1 + player.slowed()))
                            if not player.dying():
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if selected != connectedpart:
                            ...
                            if connection != None:
                                connectioncandidates.remove(
                                    (connection, connectedpart))
                            if connectedpart != None:
                                bodypartcandidates.remove(connectedpart)
                                connectionstoremove = [
                                    cn for cn in connectioncandidates if cn != None and cn[0].parent == connectedpart]
                                while len(connectionstoremove) > 0:
                                    ...
                                    connectionstoremove += [
                                        cn for cn in connectioncandidates if cn != None and cn[0].parent == cnn[1]]
                            if selected != None:
                                bodypartcandidates.append(selected)
                                for cn in selected.childconnections:
                                    connectioncandidates.append(
                                        (selected.childconnections[cn], None))
                            if connection != None:
                                connectioncandidates.append(
                                    (connection, selected))
                    ...
                    if (event.key == keybindings['list down'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list down'][0][1])) or (event.key == keybindings['list down'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list down'][1][1])):
                        ...
                        if chosen == len(player.stancesknown()) - logback:
                            logback -= 1
                        if chosen == 0:
                            logback = len(player.stancesknown()
                                          ) - logheight + 1
                    for i in range(10):
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (chosen-1) % min(7,
                                                  len(player.godsknown()) + 1)
                        ...
                        if not player.dying():
                            if chosen < len(player.godsknown()):
                                ...
                                if np.random.rand() < 0.05:
                                    gd = np.random.choice(
                                        [gd for gd in gods if not gd in player.godsknown()])
                                    player.log().append('You prayed, and someone answered!')
                                    player.godsknown().append(gd)
                                    player.log().append('You learn the ways of ' + gd.name +
                                                        ', the ' + gd.factions[0] + '-god of ' + gd.sin + '.')
                                    player.log().append(gd.pronoun.capitalize() + ' ' + gd.copula +
                    ...
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        ...
                        if isinstance(selectedspell, magic.ChoiceSpell):
                            ...
                            logback = len(selectedspell.choices(
                                player)) - logheight + 1
                            chosen = 0
                            numchosen = False
                        elif isinstance(selectedspell, magic.AreaSpell):
                            gamestate = 'spellchooselocation'
                            numchosen = False
                            logback = 0
                            chosen = 0
                            lookx = player.x
                            looky = player.y
                        elif isinstance(selectedspell, (magic.TargetedSpell, magic.BodypartTargetedSpell)):
                            gamestate = 'spellchoosetarget'
                            numchosen = False
                            logback = 0
                            chosen = 0
                            lookx = player.x
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len(selectedspell.choices(player))
                    ...
                    if (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        if len([creat for creat in cave.creatures if creat.x == lookx and creat.y == looky and creat != player]) > 0:
                            target = [creat for creat in cave.creatures if creat.x ==
                                      lookx and creat.y == looky and creat != player][0]
                            if isinstance(selectedspell, magic.TargetedSpell):
                                logback = 0
                                gamestate = 'free'
                                numchosen = False
                                updatetime(selectedspell.castingtime)
                                ...
                                gamestate = 'spellchoosetargetbodypart'
                                logback = len(selectedspell.partchoices(
                                    target)) - logheight + 1
                                chosen = 0
                                numchosen = False
                            else:  # This shouldn't happen.
                    ...
                    if (event.key == keybindings['list up'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][0][1])) or (event.key == keybindings['list up'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list up'][1][1])):
                        chosen = (
                            chosen-1) % len(selectedspell.partchoices(target))
                    ...
                    for i in range(10):
                        if event.key in numkeys[i] and i < len(selectedspell.partchoices(target)):
                            ...
                            numchosen = True
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        targetbodypart = selectedspell.partchoices(target)[
                            chosen]
                        logback = 0
                        gamestate = 'free'
                        numchosen = False
                    ...
                    for i in range(10):
                        if event.key in numkeys[i] and i < len(player.spellsknown()):
                            ...
                            numchosen = True
                    if numchosen or (event.key == keybindings['list select'][0][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][0][1])) or (event.key == keybindings['list select'][1][0] and ((event.mod & pygame.KMOD_SHIFT) == keybindings['list select'][1][1])):
                        selected = player.spellsknown()[chosen]
                        logback = 0
                        gamestate = 'free'
                        numchosen = False
                        updatetime(5)
                        if not player.dying():
                            [it for it in player.inventory if it.name ==
                ...
                if gamestate != 'dead':
                    ...
                    for i in range(mapwidth + hpbarwidth + hpmargin):
                        for j in range(mapheight + statuslines + logheight):
                            win.putchars(' ', x=i, y=j, bgcolor='black')
                    text = 'Saving the game. Please wait.'
                    win.putchars(text, x=(mapwidth + hpmargin + hpbarwidth)//2 - len(text) //
                                 2, y=(mapheight + statuslines + logheight)//2, fgcolor='white')
                    win.update()
                    with open(SAVE_PATH, 'wb') as f:
                        pickle.dump((caves, player, gods), f)
                pygame.quit()
...
def halloffame():
    ...
    while not cont:
        ...
        win.write(title, x=(mapwidth + hpbarwidth + hpmargin -
                  len(title)) // 2, y=0, fgcolor=(255, 255, 255))
        if file_exists('highscores.pickle'):
            with open('highscores.pickle', 'rb') as f:
                highscores = pickle.load(f)
        else:
        ...
        if len(highscores_sorted) > 0 and highscores_sorted.index(highscores[-1]) >= mapheight + statuslines + logheight - 1:
            ...
            listlength = min(len(highscores_sorted),
                             mapheight + statuslines + logheight - 1)
        ...
        for i in range(listlength):
            ...
            if score == highscores[-1]:
                ...
                fgcolor = (255, 255, 255)
            win.write(repr(i+1), x=0, y=i+1,
                      fgcolor=fgcolor, bgcolor=(0, 0, 0))
            win.write(repr(score[0]), x=5, y=i+1,
                      fgcolor=fgcolor, bgcolor=(0, 0, 0))
            win.write(score[1] + ', ' + score[2], x=15, y=i +
                      1, fgcolor=fgcolor, bgcolor=(0, 0, 0))
        if latestoutoftop:
            win.write('...', x=0, y=mapheight + statuslines +
                      logheight - 2, fgcolor=(255, 255, 255), bgcolor=(0, 0, 0))
            i = highscores_sorted.index(highscores[-1])
            score = highscores[-1]
            fgcolor = (0, 255, 255)
            win.write(repr(i+1), x=0, y=mapheight + statuslines +
                      logheight - 1, fgcolor=fgcolor, bgcolor=(0, 0, 0))
            win.write(repr(score[0]), x=5, y=mapheight + statuslines +
                      logheight - 1, fgcolor=fgcolor, bgcolor=(0, 0, 0))
            win.write(score[1] + ', ' + score[2], x=15, y=mapheight +
                      statuslines + logheight - 1, fgcolor=fgcolor, bgcolor=(0, 0, 0))
        win.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
    ...
    while not cont:
        for i in range(mapwidth + hpbarwidth + hpmargin):
            for j in range(mapheight + statuslines + logheight):
                win.putchars(' ', x=i, y=j, bgcolor='black')
        text = 'KEYBINDINGS'
        win.write(text, x=(mapwidth + hpbarwidth + hpmargin -
                  len(text)) // 2, y=0, fgcolor=(255, 255, 255))
        text = 'Arrow keys, return, and escape are reserved keys. Left shift can be used as a modifier.'
        win.write(text, x=(mapwidth + hpbarwidth + hpmargin -
                  len(text)) // 2, y=1, fgcolor=(255, 255, 255))
        text = 'Select with arrows, edit with return.'
        win.write(text, x=(mapwidth + hpbarwidth + hpmargin -
                  len(text)) // 2, y=2, fgcolor=(255, 255, 255))
        ...
        for i in range(len(keybindings)):
            if selected[0] == i:
                ...
                fgcolor = (255, 255, 255)
                bgcolor = (0, 0, 0)
            win.write(list(keybindings.keys())[
                      i], x=15+longest-len(list(keybindings.keys())[i]), y=i+4, fgcolor=fgcolor, bgcolor=bgcolor)
            for j in (0, 1):
                if selected == (i, j):
                    fgcolor = (0, 0, 0)
                    bgcolor = (255, 255, 255)
                ...
                if keybindings[list(keybindings.keys())[i]][j][0] in keynames:
                    keytext = shifttext + \
                        keynames[keybindings[list(
                            keybindings.keys())[i]][j][0]]
                ...
                if state == 1 and selected == (i, j):
                    keytext = '?????'
                win.write(keytext, x=15+longest+5+j*30, y=i +
                          4, fgcolor=fgcolor, bgcolor=bgcolor)
            if selected == (len(keybindings), 0):
                ...
                fgcolor = (255, 255, 255)
                bgcolor = (0, 0, 0)
            win.write('Save and return', x=15+longest+5,
                      y=len(keybindings)+5, fgcolor=fgcolor, bgcolor=bgcolor)
        if selected == (len(keybindings), 1):
            ...
            fgcolor = (255, 255, 255)
            bgcolor = (0, 0, 0)
        win.write('Reset to defaults', x=15+longest+5+30,
                  y=len(keybindings)+5, fgcolor=fgcolor, bgcolor=bgcolor)
        win.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
            ...
            if event.type == pygame.locals.KEYDOWN:
                if state == 0:
                    ...
                    if event.key == pygame.locals.K_DOWN:
                        selected = (
                            min(selected[0]+1, len(keybindings)), selected[1])
                    if event.key == pygame.locals.K_LEFT:
                    ...
                    if not event.key in reservedkeys:
                        keys = [keybindings[list(keybindings.keys())[selected[0]]][0], keybindings[list(
                            keybindings.keys())[selected[0]]][1]]
                        keys[selected[1]] = (
                            event.key, (event.mod & pygame.KMOD_SHIFT), True)
                        keybindings[list(keybindings.keys())[selected[0]]] = (
                            keys[0], keys[1])
                        for i in range(len(keybindings)):
                            for j in (0, 1):
                                if (i, j) != selected and keybindings[list(keybindings.keys())[i]][j][2] and keybindings[list(keybindings.keys())[i]][j][0] == event.key and keybindings[list(keybindings.keys())[i]][j][1] == (event.mod & pygame.KMOD_SHIFT):
                                    keys = [keybindings[list(keybindings.keys())[
                                        i]][0], keybindings[list(keybindings.keys())[i]][1]]
                                    keys[j] = (None, False, True)
                                    keybindings[list(keybindings.keys())[i]] = (
                                        keys[0], keys[1])
                        state = 0
...
def options():
    ...
    selected = 0
    fonts = (
        FONTS_PATH.joinpath('Hack-Regular.ttf'),
        FONTS_PATH.joinpath('software_tester_7.ttf'),
        FONTS_PATH.joinpath('courier-prime-sans.regular.ttf'),
        FONTS_PATH.joinpath('square.ttf'),
        FONTS_PATH.joinpath('PressStart2P-Regular.ttf'),
    )
    fontnames = (
        'Hack',
        'Software Tester 7',
    ...
    font_i = fonts.index(FONTS_PATH.joinpath(font))
    while not cont:
        for i in range(mapwidth + hpbarwidth + hpmargin):
            for j in range(mapheight + statuslines + logheight):
                win.putchars(' ', x=i, y=j, bgcolor='black')
        title = 'OPTIONS'
        win.write(title, x=(mapwidth + hpbarwidth + hpmargin -
                  len(title))//2, y=20, fgcolor=(255, 255, 255))
        ...
        if selected == 0:
            ...
            bgcolor = (0, 0, 0)
            fgcolor = (255, 255, 255)
        win.write(fonttext, x=(mapwidth + hpbarwidth + hpmargin -
                  len(fonttext))//2, y=22, fgcolor=fgcolor, bgcolor=bgcolor)
        ...
        if selected == 1:
            ...
            bgcolor = (0, 0, 0)
            fgcolor = (255, 255, 255)
        win.write(fontsizetext, x=(mapwidth + hpbarwidth + hpmargin -
                  len(fontsizetext))//2, y=24, fgcolor=fgcolor, bgcolor=bgcolor)
        showsequentiallytext = 'Show enemy movements sequentially: < ' + \
            repr(showsequentially) + ' >'
        if selected == 2:
            bgcolor = (255, 255, 255)
            fgcolor = (0, 0, 0)
        else:
            bgcolor = (0, 0, 0)
            fgcolor = (255, 255, 255)
        win.write(showsequentiallytext, x=(mapwidth + hpbarwidth + hpmargin -
                  len(showsequentiallytext))//2, y=26, fgcolor=fgcolor, bgcolor=bgcolor)
        ...
        if selected == 3:
            ...
            bgcolor = (0, 0, 0)
            fgcolor = (255, 255, 255)
        win.write(keybindingstext, x=(mapwidth + hpbarwidth + hpmargin -
                  len(keybindingstext))//2, y=28, fgcolor=fgcolor, bgcolor=bgcolor)
        ...
        if selected == 4:
            ...
            bgcolor = (0, 0, 0)
            fgcolor = (255, 255, 255)
        win.write(backtomenutext, x=(mapwidth + hpbarwidth + hpmargin -
                  len(backtomenutext))//2, y=30, fgcolor=fgcolor, bgcolor=bgcolor)
        win.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
...
def credits():
    ...
    while not cont:
        ...
        for i in range(len(credlist)):
            cred = credlist[i]
            win.write(cred, x=(mapwidth + hpbarwidth + hpmargin -
                      len(cred))//2, y=y0+10+i, fgcolor='white')
    ...
    while True:
        ...
        for i in range(len(buttontexts)):
            ...
            if i == selected:
                ...
                fgcolor = (255, 255, 255)
                bgcolor = (0, 0, 0)
            win.write(buttontexts[i], x=x, y=y,
